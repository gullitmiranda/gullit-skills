...
1. Read the source and verify whether it is a ready local plan on the shared `.agents/plans` / `.cursor/plans` tree (in Cursor, `.cursor/plans`), a tracked note, or a non-plan request.
