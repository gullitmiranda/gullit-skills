...
This document defines the durable policy for choosing **model configurations**
on a fixed orchestration surface (Cursor, Zed Agent, Claude Code, a named ACP
agent, terminal CLI, and future surfaces). Surface choice belongs to
`agent-selection`. This document describes how to make a model selection, not
...
- provider ineligibility on the surface (for example CloudWalk `llm-gateway`
...
### CursorBench shortlist (suite-stale)

Numeric CursorBench **3.2** stage hypotheses from 2026-08-22 are archived in
[`.agents/notes/current/cursorbench-3.2.research.md`](../.agents/notes/current/cursorbench-3.2.research.md).
They are **not** operating defaults. Cursor published **CursorBench 4.0** on
2026-09-10 with a different task mix and score/cost frontier; do not compare
3.2 and 4.0 scores. Refresh a Cursor shortlist only via a new dated research
note plus a local pilot on Cursor.
...
agents that can use OpenRouter (or CW gateway where configured). It is not a
...
- `zed-cloudwalk-gateway` — CW gateway wiring on Zed Agent.
