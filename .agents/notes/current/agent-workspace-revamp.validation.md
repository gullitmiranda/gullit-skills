...
- An isolated install from a conventional local clone discovered and linked all 38 current skills from the nested source-home tree, including `agent-workspace`, `agent-notes`, `work-intake`, `work-plan`, `build-plan`, `work-closeout`, and `cursor-project-path-migration`, without legacy wrappers.
