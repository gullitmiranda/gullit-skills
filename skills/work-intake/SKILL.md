...
3. Select the smallest core route: clarify or decide in the main chat; create/refine a plan with `work-plan`; execute one ready plan with `build`; coordinate multiple independent deliveries with `incremental-delivery`; or assess completed work with `work-closeout`.
...
- **Ready implementation plan**: verify its contract, then `build`; use `incremental-delivery` when independent increments are needed.
