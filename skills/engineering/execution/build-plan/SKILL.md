...
- Read `.agents/AGENTS.md` and classify the source before editing. A local implementation plan lives on the shared `.agents/plans` / `.cursor/plans` tree and is never committed. In Cursor, open it through `.cursor/plans`.
