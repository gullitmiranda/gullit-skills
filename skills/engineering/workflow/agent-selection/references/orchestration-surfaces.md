...
| Custom / employer gateway providers | No in current setup | No in current setup | Yes | Only if that agent talks to the gateway | Yes if CLI points at gateway |
| OpenRouter / multi-provider | Cursor catalog only | Cursor catalog only | Yes | Agent-dependent | Yes |
| Slack / Grok Bot / work×xAI paths | Yes | Yes | Not the center | No | No |

"Employer gateway / No in current setup" on Cursor means **eligibility today**: the
employer `llm-gateway` is not reachable as a Cursor agent provider without
...
1. Need a fine cast (multi-role, clean review, background) → **Cursor** as
   orchestrator.
2. An employer gateway or OpenRouter model is non-negotiable → **Zed Agent** (or
...
4. Slack / Grok / work×xAI agent paths → **Cursor** (local or cloud).
